...
def test_single_cell_metabolism():
    ...
    # Display results
    print("\n[GRAPH] Metabolism Results (mol/s/cell):")
    print("-" * 40)
    ...
    if oxygen_rate != 0:
        ...
        cell_volume = 15e-6 * 15e-6 * 20e-6  # 15um x 15um x 20um in m
        ...
        print("   [WARNING]  No oxygen consumption detected!")
...
def calculate_fipy_source_term(reactions: Dict[str, float]):
    ...
    print("\n[TOOL] FiPy Source Term Calculation:")
    print("-" * 40)
    ...
    print(f"   Mesh cell area: {mesh_cell_volume*1e12:.1f} um")
    ...
    for substance in ['Oxygen', 'Glucose', 'Lactate']:
        ...
        if reaction_rate != 0:
            # Convert mol/s/cell to mol/(ms) by dividing by mesh cell area
            volumetric_rate = reaction_rate / mesh_cell_volume  # mol/(ms)
            
            # Convert to mM/s for FiPy (1 mol/m = 1000 mM, but we have m so need thickness)
            # Assume 20 um thickness for 2D simulation
            thickness = 20e-6  # 20 um in meters
...
if __name__ == "__main__":
    ...
    if reactions:
        ...
        print("\n[RUN] Now run FipyTest.py to see the diffusion simulation:")
