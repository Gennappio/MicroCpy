...
def run_standalone_lactate_test(use_microc_source_terms=False):
    ...
    print(f" Cell size: {dx*1e6:.1f} x {dy*1e6:.1f} um")
    print(f" Cell volume: {(dx*dy)*1e12:.1f} um")
    ...
    if use_microc_source_terms:
        ...
        print(f"\n[SEARCH] USING EXACT MICROC SOURCE TERMS:")
...
if __name__ == "__main__":
    ...
    if results_microc['production_detected']:
        ...
        print(f"     Problem is in MicroC's FiPy integration or solver settings")
    else:
        print(f"   [!] MicroC source terms fail even in standalone")
        print(f"     Problem is in the source term magnitude")

    if results_original['production_detected'] and not results_microc['production_detected']:
        print(f"   [SEARCH] MicroC source terms are too large for FiPy stability")
    elif not results_original['production_detected'] and results_microc['production_detected']:
        print(f"   [SEARCH] Original calculation was too small")
