...
def test_microc_steady_state():
    ...
    for multiplier in source_multipliers:
        ...
        if conc_range < 0.0001:  # Very small range like MicroC
            print(f"  [TARGET] MATCHES MicroC: Nearly uniform concentrations")
        elif conc_range > 0.01:  # Significant gradients
            print(f"  [GRAPH] SIGNIFICANT GRADIENTS: Should be visible")
        else:
            print(f"  [CHART] SMALL GRADIENTS: Borderline visible")
