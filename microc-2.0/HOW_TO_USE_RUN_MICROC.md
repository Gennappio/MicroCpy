...
#### CSV File Generator
```bash
# Generate custom CSV files for 2D simulations
...
```
- **Input**: Parameters for cell generation (pattern, count, grid size)
- **Output**: CSV cell state files for 2D simulations
- **Use case**: Create human-readable cell populations for 2D testing

### 🧪 Benchmark Tools

#### FiPy VTK Reader Simulation
```bash
# Run standalone FiPy simulation on VTK cell data
python run_microc.py --fipy cell_state.vtk
```
- **Input**: VTK cell state file
- **Output**: Simulation plots saved to `benchmarks/vtk_simulation_results/`
