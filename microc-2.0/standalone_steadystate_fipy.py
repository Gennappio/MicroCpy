...
def main():
    ...
    domain_size = 1500e-6  # 1500 μm in meters
    grid_size = (75, 75)   # 75×75 grid
    nx, ny = grid_size
    ...
    cell_height = 20e-6        # 20 μm (from config)
    ...
    print(f"🔍 Lactate production rate: {lactate_production_rate:.2e} mM/s")
    ...
    print(f"🔍 After negation for FiPy:")
    print(f"   Min rate: {np.min(source_field):.2e} mM/s")
    print(f"   Max rate: {np.max(source_field):.2e} mM/s")
    ...
    print("\n🚀 Solving steady-state diffusion equation with equation.solve()...")
    ...
    if np.any(np.isnan(lactate.value)) or np.any(np.isinf(lactate.value)):
        print("\n📊 Skipping plots due to simulation divergence.")
        return None, None
    ...
    print(f"💾 Saved plot: standalone_fipy_lactate.png")
...
if __name__ == "__main__":
    ...
    if result and result[0] is not None:
        ...
        print("\n🎉 Standalone FiPy test completed!")
    else:
