...
def debug_microc_gene_states():
    ...
    key_genes = ['GLUT1', 'Cell_Glucose', 'G6P', 'PEP', 'Pyruvate', 'AcetylCoA', 'TCA', 'ETC', 'mitoATP', 'glycoATP', 'ATP_Production_Rate', 'Proliferation', 'Growth_Arrest', 'Apoptosis', 'Necrosis', 'p21']
    ...
    try:
        ...
        print(f"DEBUG: step() completed successfully")
        print(f"DEBUG: final_states type: {type(final_states)}")
        print(f"DEBUG: final_states length: {len(final_states) if final_states else 'None'}")
    ...
    print(f"\n=== ALL Available Gene States (first 20) ===")
    count = 0
    for gene_name, state in sorted(final_states.items()):
        print(f"{gene_name}: {state}")
        count += 1
        if count >= 20:
            break

    print(f"\n=== Checking Output Nodes Specifically ===")
    output_nodes = ['Growth_Arrest', 'Apoptosis', 'Necrosis', 'Proliferation', 'ATP_Production_Rate', 'mitoATP', 'glycoATP', 'p21']
    for node in output_nodes:
        if node in final_states:
            print(f"{node}: {final_states[node]}")
        else:
            print(f"{node}: NOT_IN_FINAL_STATES")

    print(f"\n=== Custom Color Function Test ===")
    # Test what the custom color function would return
    test_gene_states = {
        'mitoATP': final_states.get('mitoATP', False),
        'glycoATP': final_states.get('glycoATP', False),
        'Necrosis': final_states.get('Necrosis', False),
        'Apoptosis': final_states.get('Apoptosis', False)
    }
    print(f"Gene states for color function: {test_gene_states}")

    # Simulate the custom color function logic
    if test_gene_states.get('Necrosis', False):
        color = "black"
    elif test_gene_states.get('Apoptosis', False):
        color = "red"
    else:
        glyco_active = test_gene_states.get('glycoATP', False)
        mito_active = test_gene_states.get('mitoATP', False)
    ...
    print(f"Expected cell color: {color}")
