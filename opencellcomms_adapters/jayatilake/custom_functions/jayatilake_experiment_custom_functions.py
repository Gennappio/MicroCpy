...
def calculate_cell_metabolism(local_environment: Dict[str, float], cell_state: Dict[str, Any], config: Any = None) -> Dict[str, float]:
    ...
    # ==== OXPHOS PATHWAY (mitoATP = true) ====
    # NetLogo lines 2740-2741: Oxygen consumption
    # NetLogo line 2998: Glucose consumption = (Vmax_oxygen / 6) * MM(glucose) * MM(oxygen)
    # NetLogo line 3054: Lactate consumption = (Vmax_oxygen * 2/6) * MM(lactate) * MM(oxygen)
    if mito_atp and not glyco_atp:
        ...
        reactions['H'] = -(glucose_oxphos + lactate_oxphos)

    # ==== GLYCOLYSIS PATHWAY (glycoATP = true, mitoATP = false) ====
    # NetLogo line 3100: Glucose consumption = (Vmax_oxygen / 6) * (max_atp / 2) * MM(glucose)
    elif glyco_atp and not mito_atp:
