...
| `..._stab_close_boundary.json` | **Cell Height 20 → 30 µm** (seed unchanged) | Gain (boundary) | Cell Height is the multiplier that maps the seed's integer grid indices into physical space, and it is a GUI parameter on the World canvas. Raising it to 30 µm grows the same 1000-cell colony to ~540 µm radius in the unchanged 1500 µm domain, cutting the cell-free moat 393 → ~210 µm — with **total consumption unchanged**, isolating pure boundary geometry. Steeper near-boundary gradients pin the 4 mM line and the post-crash refill lag drops ~38 → ~12 min. (Seed coordinates are centre-relative, so the colony rescales about the domain centre and no re-centred copy of the seed is needed.) |
