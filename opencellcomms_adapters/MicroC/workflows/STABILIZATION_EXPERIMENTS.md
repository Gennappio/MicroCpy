...
| `..._stab_close_boundary.json` | Seed **1000 → 2000 cells** in the same 1500 µm domain | Gain (boundary) | The rim moves to ~505 µm radius, cutting the cell-free moat 393 → 245 µm: steeper near-boundary gradients pin the 4 mM line spatially and the post-crash refill lag drops ~38 → ~15 min. **Caveat:** total consumption also doubles, so expect a deep core permanently below threshold — the question this run answers is whether the *rim front stops travelling*, not whether the core is fed. |
