"""
Setup Space — create the discrete tile-grid world for a class-layer ABM model.

A single INITIALIZATION node that builds the Space (the grid + topology), the
...
@register_function(
    display_name="Setup Space",
    description="Create the tile-grid world (Space + Domain + Population) in the context",
...
def setup_space(env: BiologicalContext, size_x: int = 50, size_y: int = 50, tile_size: float = 1.0,
                topology_x: str = "toroidal", topology_y: str = "toroidal", seed: int = 0, **kwargs) -> bool:
    from src.abm import Domain, LatticeSpace, Population
    ...
    print(f"[setup_space] {space.nx}x{space.ny} world ({topology_x}/{topology_y})")
    return True
