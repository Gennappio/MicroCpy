#!/usr/bin/env python3
"""
Initial State Loader for OpenCellComms Simulations
...
from ..config.config import OpenCellCommsConfig
...
class InitialStateManager:
    """
    Manages loading of initial cell states for OpenCellComms simulations (VTK-based).
    """

    def __init__(self, config: OpenCellCommsConfig):
        self.config = config
    ...
    def _load_enhanced_vtk_domain(self, file_path: Path) -> Tuple[List[Dict[str, Any]], float]:
        ...
        ages = domain_data.get('ages', metadata.get('ages', []))
        generations = domain_data.get(
        ...
        for i, pos in enumerate(positions):
            ...
            cell_age = float(ages[i]) if i < len(ages) else 0.0
            cell_generation = int(generations[i]) if i < len(generations) else 0
            ...
            cell_init_data.append({
                ...
                'metabolic_state': {'value': cell_metabolism},
                'tq_wait_time': 0.0  # Default wait time
            })
