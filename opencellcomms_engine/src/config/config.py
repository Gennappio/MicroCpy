...
class OpenCellCommsConfig:
    ...
    @classmethod
    def load_from_yaml(cls, config_file: Path) -> 'OpenCellCommsConfig':
        ...
        # Resolve output directories relative to project root (not current working directory)
        # This ensures consistent output location regardless of where the script is run from
        project_root = Path(__file__).parent.parent.parent  # src/config/config.py -> project root

        if 'output_dir' in data:
            output_dir = Path(data['output_dir'])
            if not output_dir.is_absolute():
                data['output_dir'] = str(project_root / output_dir)

        if 'plots_dir' in data:
            plots_dir = Path(data['plots_dir'])
            if not plots_dir.is_absolute():
                data['plots_dir'] = str(project_root / plots_dir)

        if 'data_dir' in data:
            data_dir = Path(data['data_dir'])
            if not data_dir.is_absolute():
                data['data_dir'] = str(project_root / data_dir)

        # Resolve initial_state.file_path relative to project root
        if 'initial_state' in data and 'file_path' in data['initial_state'] and data['initial_state']['file_path']:
            ...
            if not file_path.is_absolute():
                data['initial_state']['file_path'] = str(project_root / file_path)
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'OpenCellCommsConfig':
