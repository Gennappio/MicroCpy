...
class VTKDomainExporter:
    ...
    def export_complete_domain(self, positions: np.ndarray, gene_states: Dict,
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write("# vtk DataFile Version 3.0\n")
            f.write(f"OpenCellComms Domain: {metadata.get('description', 'Generated domain')} | {metadata_str}\n")
            f.write("ASCII\n")
    ...
    def _export_logical_positions(self, positions: np.ndarray, phenotypes: List[str],
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            # VTK header with embedded metadata in description line
            f.write("# vtk DataFile Version 3.0\n")
            f.write(f"OpenCellComms Logical Positions: {metadata.get('description', 'Generated domain')} | {metadata_str}\n")
            f.write("ASCII\n")
            f.write("DATASET UNSTRUCTURED_GRID\n")
...
class VTKCellExporter:
    ...
    def export_cubic_cells(self, positions: np.ndarray, scalars: np.ndarray,
                          output_path: str, scalar_name: str = "ATP_Type",
                          title: str = "Cubic Cells from OpenCellComms",
    ...
    def export_initial_conditions(self, positions: np.ndarray, output_prefix: str = "initial_cells"):
        ...
        return self.export_cubic_cells(
            ...
            title="Initial Cubic Cells from OpenCellComms CSV Generator"
        )
    ...
    def export_simulation_state(self, cell_data: List[Tuple], output_dir: str,
        ...
        physical_result = self.export_cubic_cells(
            ...
            title=f"OpenCellComms Simulation Cells - Step {step}",
        ...
        logical_result = self.export_logical_positions(
            ...
            scalar_name=scalar_name,
            title=f"OpenCellComms Logical Positions - Step {step}"
        )
...
def export_simulation_state(population, output_dir: str, step: int,
                           cell_size_um: float) -> Optional[str]:
    """Export simulation state using comprehensive domain export (same as generation)"""
    ...
    from datetime import datetime
    metadata = {
        'description': f'OpenCellComms simulation state at step {step}',
...
def export_substance_fields(simulator, output_dir: str, step: int) -> List[str]:
    """Export substance concentration fields to VTK format"""
    exporter = VTKSubstanceFieldExporter()
    return exporter.export_substance_fields(simulator, output_dir, step)


def export_vtk_checkpoint(population, simulator, output_dir: str, step: int,
                         cell_size_um: float) -> Optional[str]:
    """
    Export unified VTK checkpoint with both cells and substances in organized folders

    This is the 3D equivalent of export_csv_checkpoint for 2D simulations.
    ...
    metadata = {
        'description': f'OpenCellComms 3D checkpoint at step {step}',
        'simulated_time': step * 0.1,  # Assuming 0.1 time units per step
        'suggested_cell_size_um': cell_size_um,
        'biocell_grid_size_um': cell_size_um,
...
def export_gene_states(population, output_dir: str, step: int) -> Optional[str]:
    """Export gene network states to H5 format"""
    import h5py
    from datetime import datetime
    ...
    with h5py.File(h5_filename, 'w') as f:
        ...
        metadata_group.attrs['version'] = "OpenCellComms-Simulation-2.0"
...
export_microc_simulation_state = export_simulation_state
export_microc_substance_fields = export_substance_fields
export_microc_vtk_checkpoint = export_vtk_checkpoint
export_microc_gene_states = export_gene_states
