#!/usr/bin/env python3
"""
CSV Export Module for OpenCellComms - 2D Simulation Results
    ...
    def export_simulation_state(self, population, output_dir: str, step: int) -> Optional[str]:
        ...
        if cells_data:
            ...
            with open(csv_filename, 'w', newline='', encoding='utf-8') as f:
                ...
                f.write(f'# OpenCellComms 2D Cell States - Step {step}\n')
    ...
    def _export_single_substance_field(self, substance_name: str, fipy_var, mesh, 
        ...
        try:
            ...
            with open(output_path, 'w', newline='', encoding='utf-8') as f:
                # Write metadata header
                f.write(f'# OpenCellComms 2D Substance Field: {substance_name} - Step {step}\n')
                f.write(f'# Generated: {datetime.now().isoformat()}\n')
...
def export_csv_cell_state(population, output_dir: str, step: int,
                         cell_size_um: float = 20.0) -> Optional[str]:
    """Export cell state to CSV format"""
    exporter = CSVCellStateExporter(cell_size_um)
    return exporter.export_simulation_state(population, output_dir, step)


def export_csv_substance_fields(simulator, output_dir: str, step: int) -> List[str]:
    """Export substance concentration fields to CSV format"""
    exporter = CSVSubstanceFieldExporter()
    return exporter.export_substance_fields(simulator, output_dir, step)


# Legacy aliases for backward compatibility
export_microc_csv_cell_state = export_csv_cell_state
export_microc_csv_substance_fields = export_csv_substance_fields
