...
def load_configuration(config_file, verbose=True):
    ...
    try:
        config = OpenCellCommsConfig.load_from_yaml(config_path)
        if verbose:
...
def setup_simulation(config, custom_functions_path=None, verbose=True):
    ...
    if config.initial_state.file_path and not hasattr(config, '_workflow_mode'):
        # Path is already resolved in config loader (relative to opencellcomms_engine root)
...
def run_workflow_mode(args):
    ...
    try:
        ...
        # --- Legacy v1.0 or v2.0 without subworkflows: use stage-based execution ---
        print(f"[WORKFLOW] Using legacy stage-based execution")
        ...
        if have_full_context:
            # Engine-driven simulation mode (full OpenCellCommspy run)
            #
