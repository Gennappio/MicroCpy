#!/usr/bin/env python3
"""
H5 File Generator for OpenCellComms
...
class H5Generator:
    def __init__(self, cell_size: float, cell_number: int, sparseness: float, 
        ...
        # Define ALL gene network nodes (from jaya.bnd - 106 nodes total)
        self.gene_nodes = [
            # Input nodes (25)
    ...
    def generate_h5_files(self, output_prefix: str = "generated_cells"):
        ...
        metadata = {
            'description': f'OpenCellComms domain with {self.cell_number} cells',
            'simulated_time': 0.0,
    ...
    def _create_cell_states_h5(self, filename: str, cell_ids: List[str], positions: np.ndarray,
        ...
        """Create cell states H5 file (same format as OpenCellComms)"""
    ...
    def _create_gene_states_h5(self, filename: str, cell_ids: List[str],
        ...
        """Create gene states H5 file (same format as OpenCellComms)"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate custom H5 cell state files for OpenCellComms",
...
Examples:
  ...
  python run_workflow.py --generate --cells 100 --radius 400 --cell-size 20 --output tumor_core

  # Generate dense core (center concentrated):
  python run_workflow.py --generate --cells 200 --radius 300 --sparseness 0.1 --cell-size 15 --output dense_core

  # Generate sparse border (border concentrated):
  python run_workflow.py --generate --cells 150 --radius 250 --sparseness 0.9 --cell-size 25 --output sparse_border
"""
    ...
    print(f"Use with: python run_workflow.py --visualize {cell_file}")
