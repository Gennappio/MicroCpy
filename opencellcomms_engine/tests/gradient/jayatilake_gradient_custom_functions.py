...
try:
    ...
    tests_dir = os.path.join(os.getcwd(), 'tests', 'gradient')
    if os.path.exists(tests_dir) and tests_dir not in sys.path:
        sys.path.insert(0, tests_dir)

    import jayatilake_gradient_custom_metabolism as metabolism
    METABOLISM_FUNCTIONS_AVAILABLE = True
    print("[+] Imported custom metabolism functions from jayatilake_gradient_custom_metabolism.py")
except ImportError as e:
    print(f"[WARNING]  Could not import custom metabolism functions: {e}")
...
def log_division_decision(cell_id: str, decision: bool, reason: str, cell_age: float, atp_rate_normalized: float, current_phenotype: str):
    """
    Log cell division decisions to the main simulation status log file.
    """
    try:
        # Create logs directory if it doesn't exist
        logs_dir = Path("results") / "gradient_experiment"
        logs_dir.mkdir(parents=True, exist_ok=True)

        # Log to the main simulation status log file
        log_file = logs_dir / "log_simulation_status.txt"

        # Append decision log
        with open(log_file, 'a', encoding='utf-8') as f:
            decision_str = "DIVIDE" if decision else "NO_DIV"
            f.write(f"DIVISION: {cell_id[:8]} | {decision_str} | Age={cell_age:6.1f} | ATP={atp_rate_normalized:6.3f} | {current_phenotype:12s} | {reason}\n")

    except Exception as e:
        # Don't let logging errors crash the simulation
        print(f"Warning: Failed to log division decision: {e}")
...
def custom_initialize_cell_placement(grid_size: Tuple[int, int], simulation_params: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    domain_size_um = simulation_params.get('domain_size_um', 600.0)  # Default 600 um
    cell_height_um = simulation_params.get('cell_height_um', 20.0)   # Default 20 um
    ...
    print(f"[SEARCH] BIOLOGICAL CELL GRID DEBUG:")
                ...
                if distance <= radius:
                    ...
                    if fipy_pos in used_positions:
                        ...
                        if collision_count <= 5:
                            print(f"   [WARNING]  COLLISION: Cell {cells_placed} at bio({x},{y}) -> fipy({fipy_x},{fipy_y}) already used!")
    ...
    # Summary
    print(f"   [CHART] Placement summary: {len(placements)} cells placed, {collision_count} collisions avoided")
...
def custom_calculate_cell_metabolism(local_environment: Dict[str, float], cell_state: Dict[str, Any], config: Any = None) -> Dict[str, float]:
    ...
    reactions = {
        ...
        # Growth factors (gammaS,C x CS consumption, gammaS,P production)
        'FGF': 0.0,
        'TGFA': 0.0,
        'HGF': 0.0,
    ...
    # Based on NetLogo OpenCellComms implementation (lines 2788, 2998, 3054, 3161)
    ...
    if glyco_atp > 0:
        ...
        # Formula: R_L,p = (2*u_O2*A_0/6) * (C_G/(2*K_G + C_G)) * (glycoATP)
        # Where u_O2*A_0 = vmax_oxygen, K_G = km_glucose, C_G = local_glucose
    ...
    # Growth factor kinetics (Jayatilake et al. 2024: RS = gammaS,C x CS for consumption, RS = gammaS,P for production)
...
def custom_should_divide(cell, config: Any) -> bool:
    ...
    atp_rate_normalized = 0.0  # Not used in gradient experiment
    ...
    if current_phenotype != "Proliferation":
        decision = False
        reason = f"Not proliferating: phenotype={current_phenotype}"
    else:
        ...
        if cell_age < cell_cycle_time:
            decision = False
            reason = f"Age insufficient: {cell_age:.1f} < {cell_cycle_time}"
        else:
            # Additional checks could be added here based on gene states
            # For now, use simple age-based division for proliferative cells
            decision = True
            reason = f"Age sufficient: {cell_age:.1f} >= {cell_cycle_time}, phenotype={current_phenotype}"

    # Log the division decision if logging is enabled
    if hasattr(config, 'log_simulation_status') and config.log_simulation_status:
        log_division_decision(cell.state.id, decision, reason, cell_age, atp_rate_normalized, current_phenotype)
...
def custom_final_report(population, local_environments, config: Any = None) -> None:
    ...
    print(f"\n[CHART] POPULATION SUMMARY ({len(population.cells)} total cells):")
