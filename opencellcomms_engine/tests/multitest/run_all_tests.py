...
from simulation.simulator import OpenCellCommsSimulator
...
def run_single_test(config_file, combination_id):
    ...
    try:
        # Load configuration
        config = OpenCellCommsConfig.load_from_yaml(Path(config_file))
        print("[+] Config loaded")
        ...
        # Create simulator
        simulator = OpenCellCommsSimulator(config)
