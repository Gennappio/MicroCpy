...
def world():
    return LatticeWorld(5, 5, 1, "bounded", "bounded")


def test_at_and_set():
    r = FieldResource("s", world(), initial=2.0)
    assert r.at((1, 1)) == 2.0
    r.set_at((1, 1), 9.0)
    assert r.at((1, 1)) == 9.0
...
def test_deposit_sink_clamps_at_zero():
    r = FieldResource("s", world(), initial=3.0)
    r.deposit((1, 1), -10.0)
    r.apply_sources()
...
def test_deposit_source():
    r = FieldResource("s", world(), initial=0.0)
    r.deposit((0, 0), 5.0)
    r.apply_sources()
    assert r.at((0, 0)) == 5.0
...
def test_totals():
    r = FieldResource("s", world(), initial=1.0)
