#!/usr/bin/env python3
"""
Standalone FiPy test that exactly matches OpenCellComms's approach.
Uses same parameters, same cell distribution, same calculations as OpenCellComms.
"""
...
def main():
    print(" Standalone FiPy Test - Exact OpenCellComms Match")
    print("=" * 50)
    
    # EXACT OpenCellComms parameters from config
    ...
    # Set boundary conditions (fixed boundaries like OpenCellComms)
    ...
    # Place 200 cells in center using OpenCellComms's approach
    ...
    # Use expanding circle pattern like OpenCellComms
    radius = 1
    cells_placed = 0
    max_cells = 1
    ...
    # Create source field with EXACT OpenCellComms calculation
    print("\n Creating source field with OpenCellComms calculation...")
    
    source_field = np.zeros(nx * ny)
    
    # OpenCellComms volume calculation
    mesh_cell_volume = dx * dy * cell_height
    twodimensional_adjustment_coefficient = 1.0  # From OpenCellComms config
