...
"""
===============================================================================
GRAPH-WALKING GENE NETWORK SIMULATOR WITH PROBABILISTIC INPUT ACTIVATION
===============================================================================

This version extends gene_network_netlogo_faithful.py to include the 
probabilistic activation mechanism for GLUT1I and MCT1I nodes, exactly as 
implemented in the NetLogo model.
...
Basic usage:
    python gene_network_netlogo_probability.py network.bnd inputs.txt --runs 100 --steps 500
...
===============================================================================
PROBABILISTIC INPUT ACTIVATION (NEW IN THIS VERSION)
===============================================================================
...
The Hill function creates a sigmoidal response curve, with 85% maximum
activation probability at high concentrations.

Visual example of the Hill function response:
    concentration    probability    outcome (if cell_ran = 0.5)
    0.0              0.0%           OFF
    0.5              29.8%          OFF (29.8% < 50%)
    1.0              42.5%          OFF (42.5% < 50%)
    2.0              56.7%          ON  (56.7% > 50%)
    5.0              73.9%          ON  (73.9% > 50%)
    10.0             80.9%          ON  (80.9% > 50%)
    ∞                85.0%          ON  (85.0% > 50%)

Two cells with concentration=2.0 but different random values:
    Cell A: ran1=0.3 → 56.7% > 30% → MCT1I = ON
    Cell B: ran1=0.7 → 56.7% < 70% → MCT1I = OFF
This is the source of cell-to-cell variability.
...
class NetLogoFaithfulGeneNetwork:
    ...
    *** CUSTOMIZE UPDATE MECHANISM HERE ***
    ...
    def __init__(self):
        ...
        # Used by MCT1I and GLUT1I for stochastic activation (NetLogo lines 1014-1020, 1298-1321)
...
def main():
    parser = argparse.ArgumentParser(
        description='NetLogo Gene Network Simulator with Probabilistic Input Activation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Standard run with probabilistic GLUT1I/MCT1I
  python gene_network_netlogo_probability.py network.bnd inputs.txt --runs 100 --steps 500

  # Non-reversible mode (stop at first fate)
  python gene_network_netlogo_probability.py network.bnd inputs.txt --runs 100 --steps 500 --non-reversible

  # With cell actions layer (proliferation/death events)
  python gene_network_netlogo_probability.py network.bnd inputs.txt --runs 100 --steps 500 --apply-cell-actions
  
  # With seed for reproducibility (each run gets different cell random values)
  python gene_network_netlogo_probability.py network.bnd inputs.txt --runs 100 --steps 500 --seed 42
        """
    )
